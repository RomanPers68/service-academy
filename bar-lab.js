...
export function sequenceFromSteps(c) {
  ...
  const out = []; let vessel = method === "шейк" ? "shaker" : method === "стир" ? "mixing" : method === "блендер" ? "blender" : "glass";
  // Доп. 252: сосуд — по словам карточки, а не по методу. «В рокс — сироп и биттер»
  // (Олд фэшн) значит сборку прямо в бокале, даже если метод «стир»: иначе тренажёр
  // просил бы смесительный стакан там, где карточка его не называет.
  {
    const s0 = norm(steps[0] || "");
    // «стакан» у бармена — смесительный, не бокал гостя; и если карточка где-то
    // просит процедить, сборка точно шла НЕ в бокале.
    const willStrain = steps.some(t => /стрейн|процед|перели/.test(norm(t)));
    const inGlass = /(^|[^а-я])в\s+(рокс|бокал|хайбол|купе|шот|флюте|харрикейн|маргарит|айриш)/.test(s0) && !/смесительн/.test(s0) && !willStrain;
    if (inGlass) vessel = "glass";
    else if (/смесительн|(^|[^а-я])в\s+стакан/.test(s0)) vessel = "mixing";
    else if (/шейкер/.test(s0)) vessel = "shaker";
    else if (willStrain && vessel === "glass") vessel = method === "шейк" ? "shaker" : "mixing";
  }
  const vessel0 = vessel;
  ...
  steps.forEach((text, si) => {
    ...
    let usedText = false;                 // текст шага достаётся первому из его действий
    for (const { s: st } of toks) {
      // Текст шага достаётся ПЕРВОМУ действию; остальные получают короткое имя.
      // Лёд при этом всегда называется льдом, даже если он первый в тексте, —
      // иначе «Добавь джин, лимон, лёд — шейк» выглядит как действие, а не лёд.
      if (st.kind === "ice") st.label = (st.id === "crushed" ? "Краш" : "Лёд") + (st.where === "shaker" ? " в шейкер" : st.where === "mixing" ? " в смесительный стакан" : st.where === "blender" ? " в блендер" : " в бокал");
      else if (st.kind === "tool") { if (usedText) st.label = TOOLS[st.id] || st.label; else usedText = true; }
