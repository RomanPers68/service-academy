...
// ── Доп. 240: свои коктейли со спеком — записи kind:"cocktail" в меню команды (едут через menu_get/menu_set)
export const isCocktailRec = (d) => d && d.kind === "cocktail" && d.id;
export const cocktailRecords = (shared) => (shared || []).filter(isCocktailRec);
export function withCocktail(shared, rec) { const rest = (shared || []).filter(d => !(isCocktailRec(d) && d.id === rec.id)); return [...rest, { ...rec, kind: "cocktail", updatedAt: Date.now() }]; }
export function withoutCocktail(shared, id) { return (shared || []).filter(d => !(isCocktailRec(d) && d.id === id)); }
/** Запись редактора → объект коктейля для Колоды и Сборки (тот же формат, что data/cocktails.js) */
export function recToCocktail(r) {
  return { id: r.id, house: true, full: true, name: r.name, base: r.base || "", glass: r.glass || "rocks", color: Array.isArray(r.color) && r.color.length === 2 ? r.color : ["#D6B266", "#A8823A"],
    ice: r.ice === "crushed" ? "crushed" : r.ice === "none" ? false : true, garnish: r.garnish || "none", strength: Number(r.strength) || 3, sweet: Number(r.sweet) || 2, method: r.method || "билд",
    ing: (r.ing || []).filter(i => i && i[0]).map(i => [i[0], i[2] === "доверху" ? (Number(i[1]) || 90) : (Number(i[1]) || i[1] || ""), i[2] && i[2] !== "мл" ? i[2] : undefined].filter((v, k) => k < 2 || v !== undefined)),
    steps: (r.steps || []).filter(Boolean), tip: r.tip || "", pair: r.pair || "", note: r.note || "", story: r.story || "", short: r.short || "", fizz: !!r.fizz, foam: !!r.foam, inCard: r.inCard !== false, stop: r.stop || null, img: r.img || "" };
}
export function isFullCocktail(c) { return !!(c && c.full && (c.ing || []).length && (c.steps || []).length); }
...
export function houseCocktails(dishes) {
  const full = cocktailRecords(dishes).filter(r => !r.archived).map(recToCocktail);
  const legacy = (dishes || []).filter(d => d && d.name && !d.kind && !d.archived && /коктейл|бар|напит/.test(norm(d.cat))).map(d => {
...
export function dishFaq(d) {
  ...
  const nut = dishNutrition(d);                                   // Доп. 264
  if (nut) pool.push({ q: "Сколько калорий?", a: `${nut.kcal} ккал на порцию ${nut.out ? nut.out + " г" : ""} — белки ${nut.p}, жиры ${nut.f}, углеводы ${nut.c}.`.replace(/\s+/g, " ") });
...
// ── Доп. 244: семейства коктейлей — учатся кластерами, а не по одному
export const FAMILIES = [
  { id: "negroni", title: "Негрони и родня", note: "Одна формула — равные части крепкого, горького и сладкого. Меняешь базу — меняется характер.", ids: ["negroni", "boulevardier", "aperol-spritz"] },
  { id: "sour", title: "Сауэры", note: "Крепкое + кислое + сладкое — и на этом стоит половина карты. Освой один, узнаешь все.", ids: ["whiskey-sour", "daiquiri", "margarita", "gimlet", "amaretto-sour", "cherry-sour", "ny-sour", "aviation", "basil-smash", "bramble", "top-grade", "honey-pear"] },
  { id: "stir", title: "Крепкие стиры", note: "Только алкоголь, лёд и терпение: 20–30 секунд стира — и прозрачный холод.", ids: ["old-fashioned", "manhattan", "martini", "gibson", "rusty-nail", "negroni", "boulevardier"] },
  { id: "highball", title: "Хайболы", note: "Крепкое + газировка + лёд: самый быстрый и самый честный жанр.", ids: ["gin-tonic", "moscow-mule", "cuba-libre", "paloma", "tom-collins", "mojito", "sea-breeze", "blue-lagoon", "long-island"] },
  { id: "dessert", title: "Десертные", note: "Сливки, ликёры, кофе — вместо десерта или к нему.", ids: ["white-russian", "black-russian", "grasshopper", "brandy-alexander", "pina-colada", "espresso-martini", "irish-coffee", "b52", "hiroshima", "stinger"] },
  { id: "tiki", title: "Тики и тропики", note: "Ром, соки, специи — карнавал в бокале, гарниш на всю длину.", ids: ["mai-tai", "hurricane", "jungle-bird", "pina-colada", "strawberry-daiquiri", "sex-on-the-beach", "tequila-sunrise", "bloody-passion"] },
  { id: "sparkling", title: "С игристым", note: "Флюте, холод, минимум движений — и бокал поднимает вечер.", ids: ["bellini", "french-75", "aperol-spritz", "russian-spring-punch"] },
];
const jaccard = (a, b) => { const A = new Set(a), B = new Set(b); const i = [...A].filter(x => B.has(x)).length; return i / (A.size + B.size - i || 1); };
/** Родня коктейля: из кураторских семейств, а для своих и одиночек — по сходству состава и метода */
export function familyOf(c, all) {
  const fams = FAMILIES.filter(f => f.ids.includes(c.id));
  const fromFam = fams.flatMap(f => f.ids).filter(id => id !== c.id);
  const byId = new Map((all || []).map(x => [x.id, x]));
  const rel = [...new Set(fromFam)].map(id => byId.get(id)).filter(Boolean);
  if (rel.length >= 2) return { title: fams[0].title, note: fams[0].note, items: rel.slice(0, 6) };
  const my = (c.ing || []).map(i => norm(i[0]).split(" ").pop());
  const scored = (all || []).filter(x => x.id !== c.id).map(x => ({ x, s: jaccard(my, (x.ing || []).map(i => norm(i[0]).split(" ").pop())) + (x.method === c.method ? 0.25 : 0) + (x.glass === c.glass ? 0.1 : 0) })).filter(o => o.s > 0.3).sort((a, b) => b.s - a.s).slice(0, 4).map(o => o.x);
  return scored.length ? { title: "Похожие по составу", note: "Подобраны по общим ингредиентам и методу.", items: scored } : null;
}

// ── Доп. 244: «Гость спрашивает» по-барному — с заменой из карты
export function cocktailFaqSmart(c, all, inCard) {
  const base = cocktailFaq(c);
  const pool = (all || []).filter(x => x.id !== c.id && (!inCard || inCard(x)) && !x.stop);
  const sameBase = (x) => c.base && x.base ? norm(x.base) === norm(c.base) : true;
  const out = [];
  if ((c.sweet || 0) >= 2) { const alt = pool.filter(x => (x.sweet || 0) <= 1 && sameBase(x)).sort((a, b) => (a.sweet || 0) - (b.sweet || 0))[0]; out.push({ q: "Есть что-то посуше?", a: alt ? `Да — «${alt.name}»: та же база, без сладости.` : "Здесь сладость держит баланс; из карты посуше — спроси у бармена.", go: alt && alt.id }); }
  if ((c.strength || 0) >= 3) { const alt = pool.filter(x => (x.strength || 0) <= 2 && (sameBase(x) || x.method === c.method))[0]; out.push({ q: "Похожее, но полегче?", a: alt ? `«${alt.name}» — легче и в том же духе.` : "Полегче — предложи хайбол на той же базе.", go: alt && alt.id }); }
  const zero = pool.find(x => (x.strength || 0) === 0 || /безалко|мокт|zero|0\.0/.test(norm(x.name)));
  out.push({ q: "А без алкоголя?", a: zero ? `Есть — «${zero.name}».` : "Безалкогольной версии в карте нет — предложи домашний лимонад или тоник с тем же гарнишем.", go: zero && zero.id });
  return [...out, ...base].slice(0, 4);
}
