...
export function splitExtras(questions) {
  ...
  return {
    quiz: all.filter(q => !(q && ["situation", "dialogue", "build", "review"].includes(q.kind))),
    review: all.find(q => q && q.kind === "review") || null,   // страница книги отзывов (правка 166)
...
export function buildCustomModules(customLessons, role, { color, icon } = {}) {
  ...
  return sectionsOf(customLessons, role).map(({ name, records }) => ({
    ...
    // отзыв гостя для книги — в том же виде, что у штатных модулей ({ text, guest, table })
    review: (() => { const rv = records.map(c => splitExtras(c.questions).review).find(x => x && (x.text || "").trim());
      return rv ? { text: rv.text.trim(), guest: (rv.guest || "").trim() || "Гость", table: (rv.table || "").trim() } : null; })(),
  }));
}
