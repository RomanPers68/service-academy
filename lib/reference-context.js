...
function fieldHit(field, st, minLen = 3) {
  ...
  return norm(field).split(/[^а-яa-z0-9]+/).some(fw => fw && use.some(x => fw.startsWith(x)));
}
...
export function menuAssistantContext(question, restaurant) {
  ...
  const parts = scored.slice(0, 2).map(({ d }) => {
    let card = `— Блюдо «${d.name}»${d.cat ? ` (${d.cat})` : ""}. Состав: ${(d.ingredients || []).join(", ") || "—"}. Аллергены: ${(d.allergens || []).join(", ") || "нет в списке"}.`;
 ...
 */
export function withRefContext(messages, profile) {
  if (!messages.length) return messages;
  const last = messages[messages.length - 1];
  if (last.role !== "user") return messages;
  const menuCtx = menuAssistantContext(last.content, profile?.restaurant);
  // Меню в приоритете: если вопрос про блюдо, справочник берём одной главой
  let refCtx = refAssistantContext(last.content);
  if (menuCtx && refCtx) refCtx = refCtx.split("\n\n")[0];
  if (!menuCtx && !refCtx) return messages;
  let instr = "\n\n[Данные из разделов приложения — отвечай по ним уверенно, это твой же ресторан. ";
  if (menuCtx) instr += "Состав и аллергены блюд взяты из раздела «Меню»; кнопку перехода ставь маркером [[go:menu|Открыть меню]]. ";
  if (refCtx) instr += "По темам Справочника кнопка — [[go:reference|Открыть Справочник]] (не go:glossary). ";
  if (menuCtx && refCtx) instr += "Выбери ОДНУ кнопку по теме вопроса. ";
  const body = [menuCtx, refCtx].filter(Boolean).join("\n\n");
  const enriched = last.content + instr + "\n" + body.slice(0, 1800) + "]";
