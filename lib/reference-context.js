...
export function cocktailAssistantContext(question) {
  ...
  return scored.slice(0, 2).map(({ c }) => {
    ...
    const cardNote = bc && !bc.includes(c.id) ? " ⚠ НЕ В КАРТЕ БАРА — в этом ресторане не наливают." : "";
