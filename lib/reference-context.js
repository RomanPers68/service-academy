...
export function dishesOf(restaurant) {
  ...
  const team = (shared[restaurant] || []).filter(d => d && d.name && !ownIds.has(d.id));
...
export function rememberSharedMenu(restaurant, dishes) {
  ...
  try {
    ...
    shared[restaurant] = dishes.slice(0, 120);
...
export function menuAssistantContext(question, restaurant) {
  ...
  const parts = scored.slice(0, 2).map(({ d }) => {
    let card = `— Блюдо «${d.name}»${d.cat ? ` (${d.cat})` : ""}${d.id ? ` [id: ${d.id}]` : ""}. Состав: ${(d.ingredients || []).join(", ") || "—"}. Аллергены: ${(d.allergens || []).join(", ") || "нет в списке"}.`;
...
export function withRefContext(messages, profile, learner) {
  ...
  if (menuCtx) instr += "Состав и аллергены блюд взяты из раздела «Меню»; кнопка — [[go:menu|Открыть меню]]. ";
  ...
  if (menuCtx || ckCtx) instr += "Когда рассказываешь о конкретном коктейле или блюде из данных, добавь отдельной строкой маркер с его id — [[cocktail:ID]] или [[dish:ID]] — приложение покажет карточку с картинкой. Не больше двух маркеров, только id из данных. ";
