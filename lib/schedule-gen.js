      ...
      slots.forEach(sl => {
        ...
        const cand = staff.filter(s => canAssign(s) && s.pos === sl.pos && canPlace(pm, s, d, sl.k))
          .sort((a, b) => {
            ...
            // Серия одинаковых смен подряд. Справедливость считалась только по
            // СУММЕ за месяц, а на вчерашний день не смотрел никто: при тонком
            // пуле кандидатов (выходные по дням недели, отпуска, «не смогу»)
            // одному и тому же человеку доставалось 3–4 утра подряд. Для денег
            // это ощутимо: утро 6 ч против дня 13 ч. Считаем, сколько дней
            // подряд человек уже выходит в ЭТУ смену, и отдаём слот тому, у
            // кого серия короче. Потолок 2 — чтобы ключ гасил третью подряд,
            // а обычное чередование решали прежние ключи.
            if (sl.k !== baseK) {
    ...
    for (let sweep = 0; sweep < repairSweeps; sweep++) {
      for (let d = 1; d <= DAYS; d++) {
        for (const sl of daySlots(pm, d)) {
          ...
          let s = staff.find(x => canAssign(x) && x.pos === sl.pos && !wished(x.id, d) && canPlace(pm, x, d, sl.k))
               || staff.find(x => canAssign(x) && x.pos === sl.pos && canPlace(pm, x, d, sl.k));
          let kFill = sl.k;
          if (!s && sl.flex) {
