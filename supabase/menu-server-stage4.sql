...
-- Подтверждение: сколько функций меню-сервера реально создано (ожидаем 4).
select 'меню-сервер: ' || count(*) || ' функции из 4' as result
from pg_proc p join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in ('menu_get','menu_set','menu_progress_set','menu_progress_list');
