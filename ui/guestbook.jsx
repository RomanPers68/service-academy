    ...
    <div style={T.screen} className="sa-screen">
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "12px 16px 6px" }}>
        <div style={{ position: "relative" }} onTouchStart={onSwipeStart} onTouchEnd={onSwipeEnd}>
          ...
          <div style={{ position: "absolute", inset: "8px -5px -7px -5px", borderRadius: 14, background: "#E8DEC8", transform: "rotate(-1.1deg)", opacity: .45 }} />
          ...
          <div key={tab + ":" + idx} className={dir === "r" ? "gb-page-r" : "gb-page-l"}
            ...
            {page.kind === "challenge" ? (
              <div style={{ position: "relative", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 10 }}>
                  ...
                  {buildLessons.map(l => {
                    ...
                    return (
                      <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px dashed rgba(122,101,72,.2)" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ color: INK, fontSize: 12.5, fontFamily: "Georgia, serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {(l.title || "").replace(/^Сборка:\s*/i, "")}
                          </div>
                          <div style={{ ...MONO, color: col, fontSize: 9, letterSpacing: 1.5, marginTop: 2, textTransform: "uppercase" }}>{verdict}</div>
                        </div>
              ...
              <div style={{ position: "relative", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 10, opacity: .88 }}>
                {page.legend
                  ? <div style={{ width: 52, height: 52, borderRadius: "50%", border: `1.5px dashed ${BROWN}`, display: "flex", alignItems: "center", justifyContent: "center", color: BROWN, fontSize: 21 }}>✦</div>
                  ...
                  {page.kind === "legend" && (
                    <div className="gb-seal" style={{ width: 58, height: 58, borderRadius: "50%", flexShrink: 0, background: `radial-gradient(circle at 34% 30%, #B0492F, ${WAX} 62%, #5E1F12)`, boxShadow: "0 4px 10px rgba(94,31,18,.45), inset 0 2px 4px rgba(255,255,255,.22), inset 0 -3px 6px rgba(0,0,0,.35)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ width: 43, height: 43, borderRadius: "50%", border: "1px solid rgba(255,220,190,.4)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2 }}>
                        ...
                        <span style={{ ...MONO, color: "#EFE4C8", fontSize: 9, letterSpacing: 1 }}>{page.seal}</span>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {pages.length > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12, padding: "0 4px" }}>
            <button onClick={() => go(-1)} disabled={idx === 0} style={{ background: "transparent", border: `1px solid ${idx === 0 ? "rgba(120,100,60,.25)" : GOLD + "66"}`, color: idx === 0 ? "rgba(120,100,60,.4)" : GOLD, borderRadius: 18, padding: "6px 15px", fontSize: 14, cursor: idx === 0 ? "default" : "pointer", fontFamily: "Georgia, serif" }}>‹</button>
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center", maxWidth: 220 }}>
              {pages.map((p, i) => {
                ...
                <div key={p.key} onClick={() => { if (i !== idx) vibrate("light"); setDir(i > idx ? "r" : "l"); setIdx(i); }}
                  style={{ width: i === idx ? 16 : 6, height: 6, borderRadius: 3, cursor: "pointer", transition: "all .25s ease", background: i === idx ? GOLD : p.kind === "locked" ? (a11y ? "rgba(120,100,60,.3)" : "#3A2E1C") : p.kind === "legend" ? WAX : "rgba(200,169,110,.45)", boxShadow: isUnread ? `0 0 7px ${GOLD}` : undefined }} />
                );
              })}
            </div>
            <button onClick={() => go(1)} disabled={idx === pages.length - 1} style={{ background: "transparent", border: `1px solid ${idx === pages.length - 1 ? "rgba(120,100,60,.25)" : GOLD + "66"}`, color: idx === pages.length - 1 ? "rgba(120,100,60,.4)" : GOLD, borderRadius: 18, padding: "6px 15px", fontSize: 14, cursor: idx === pages.length - 1 ? "default" : "pointer", fontFamily: "Georgia, serif" }}>›</button>
          </div>
        )}
