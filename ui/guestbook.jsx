...
export function NewPageBanner({ T, mod, completed, quizDone, onOpen }) {
  ...
  return (
    <div className="sa-bookbanner" style={{
      position: "sticky", top: "calc(env(safe-area-inset-top, 0px) + 56px)", zIndex: 60,
      margin: "12px 16px 0", borderRadius: 18, padding: "12px 14px",
      display: "flex", alignItems: "center", gap: 12,
      background: lt ? "rgba(252,246,232,0.92)" : "rgba(34,25,12,0.88)",
      backdropFilter: "blur(10px) saturate(1.15)", WebkitBackdropFilter: "blur(10px) saturate(1.15)",
      border: lt ? "1px solid rgba(160,120,50,0.65)" : "1px solid rgba(220,180,95,0.85)",
      borderTop: lt ? "1px solid rgba(255,250,235,0.95)" : "1px solid rgba(235,200,120,0.95)",
      boxShadow: lt
        ? "0 12px 28px rgba(120,90,30,0.22), 0 0 22px rgba(200,160,80,0.30), 0 1px 0 rgba(255,250,235,0.7) inset"
        : "0 14px 34px rgba(0,0,0,0.55), 0 0 26px rgba(212,168,90,0.38), 0 1px 0 rgba(220,180,95,0.35) inset",
      transform: leaving ? "translateY(-16px)" : undefined,
      opacity: leaving ? 0 : undefined,
      transition: "transform 0.35s ease, opacity 0.35s ease" }}>
      <div className="sa-bookicon" style={{ width: 42, height: 42, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 21,
      ...
      <style>{`
        ...
        @keyframes saBookGlow { from { filter: drop-shadow(0 0 0px rgba(212,168,90,0)); } to { filter: drop-shadow(0 0 14px rgba(212,168,90,0.55)); } }
        @keyframes saBookWiggle { 0%, 86% { transform: rotate(0deg); } 90% { transform: rotate(-8deg); } 94% { transform: rotate(8deg); } 98% { transform: rotate(-4deg); } 100% { transform: rotate(0deg); } }
        .sa-bookbanner { animation: saBookIn 0.6s cubic-bezier(0.22,1,0.36,1) both, saBookGlow 1.6s ease-in-out 0.6s infinite alternate; will-change: transform, opacity, filter; }
        .sa-bookbanner .sa-bookicon { animation: saBookWiggle 3.2s ease-in-out 1.2s infinite; }
        @media (prefers-reduced-motion: reduce) { .sa-bookbanner, .sa-bookbanner .sa-bookicon { animation-duration: 0.01s; animation-iteration-count: 1; } }
      `}</style>
    </div>
  );
}
